...
class UserListComponent extends Component{
    /* 생성자로 유저리스트컴포넌트가 만들어질 때, 부모의 파라미터값을 받아오고, 유저를 배열로만들고 메시지는 null로 만들어줘 */
    constructor(props){
        super(props);
    ...
    /* 만약 컴포넌트 디드마운트가 실행되면 reloadUserList메서드를 실행시켜줘 */
    componentDidMount(){
        this.reloadUserList();
    }
    ...
    reloadUserList = () => {
        ApiService.fetchUsers()
            ...
            /* 만약에 reloadUserList메서드를 실행할 때, 에러가 뜨면 콘솔창에 에러를 찍어줘 */
            .catch(err =>{
    ...
    deleteUser = (userID) =>{
        ApiService.deleteUser(userID)
            ...
            this.setState({
                /* 만약에 정상적으로 deleteUser를 실행하면 잘지웠다고 메세지를 넣어줘 */
                ...
                <TableBody>
                    {this.state.users.map(user =>
                        <TableRow key ={user.id}>
                            ...
                            <TableCell align="right" onClick={()=>this.deleteUser(user.id)}><DeleteIcon/>
                            </TableCell>
                        </TableRow>
                        )}
                </TableBody>
